...
class clean:
    ...
    def fit(self, string):
        ...
        for key, reg in regex.items():
            string = regex[key].sub(lambda m: " <" + key + "> ",
                                    string)
        for word in self.emoticons.keys():
            string = string.replace(word, self.emoticons[word])
        string = string.lower()
        for word in self.contraction_mapping.keys():
            string = string.replace(word, self.contraction_mapping[word])
        string = re.sub(r"[\-\"`@#$%^&*(|)/~\[\]{\}:;+,._='!?]+", " ", string)
        string = unicodedata.normalize('NFKD', string).encode('ascii', errors='ignore').decode('utf8',
                                                                                               errors='ignore')
        string = re.sub(r'\b([b-hB-Hj-zJ-Z] )', ' ', string)
        string = re.sub(r'( [b-hB-Hj-zJ-Z])\b', ' ', string)
